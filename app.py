...
# 必須放在最首行
st.set_page_config(page_title="專業證書生成器 V5.5", layout="wide")
...
if not bg_file or not data_file:
    st.info("💡 請上傳圖片和資料。使用左側「側邊欄」調整參數（側邊欄邊框可滑鼠拖動）。")
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("🔗 Photoshop 批量連結工具", expanded=True):
        st.session_state.linked_layers = st.multiselect("選取要同步的欄位", display_cols)
        lc1, lc2 = st.columns(2)
        with lc1: bx = st.number_input("X 位移", value=0, key="batch_input_x")
        with lc2: by = st.number_input("Y 位移", value=0, key="batch_input_y")
        bs = st.number_input("字體縮放", value=0, key="batch_input_s")
        ...
        if st.button("✅ 執行批量套用", use_container_width=True):
            for c in st.session_state.linked_layers:
                ...
                # 2. 【關鍵修正】同步更新各組件的 Session State Key
                # 這樣在 Rerun 時，Slider 和 Number Input 就會讀取到正確的新值
    ...
    for col in display_cols:
        ...
        with st.expander(f"圖層：{col}{link_tag}"):
            ...
            # 綁定 Key 並確保初始值與 settings 同步
            s["x"] = st.slider(f"X 位置", 0, W, float(s["x"]), key=f"x_{col}")
            s["y"] = st.slider(f"Y 位置", 0, H, float(s["y"]), key=f"y_{col}")
...
zoom = st.slider("🔍 畫布視覺縮放 (%)", 50, 250, 100, step=10, key="canvas_zoom_slider")
...
if not target_df.empty:
    ...
    for col in display_cols:
        ...
        # 輔助線
        lc = "#FF0000BB" if col in st.session_state.linked_layers else "#0000FF44"
        draw.line([(0, s["y"]), (W, s["y"])], fill=lc, width=2)
        draw.line([(s["x"], 0), (s["x"], H)], fill=lc, width=2)
