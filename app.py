...
# 1. 系統初始化與頁面設定
# ==========================================
st.set_page_config(page_title="專業證書生成器 V7.5 穩定修復版", layout="wide")
...
if not bg_file or not data_file:
    st.info("👋 上傳背景圖與資料檔後即可開始。V7.5 已修復批量套用時的系統崩潰錯誤。")
    st.stop()
...
with st.sidebar:
    ...
    for col in display_cols:
        ...
        with st.expander(f"圖層：{col}{link_tag}"):
            ...
            st.caption(f"📍 中心參考值：X={mid_x:.0f}, Y={mid_y:.0f}")
            ...
            f1, f2 = st.columns(2)
            with f1: s["size"] = st.number_input("字體大小", 10, 5000, int(s["size"]), key=f"size_{col}")
...
search_q = st.text_input("🔍 搜尋名單過濾...", placeholder="輸入關鍵字...")
view_df = st.session_state.selection_df.copy()
if search_q: view_df = view_df[view_df.index.str.contains(search_q, case=False)]

edited_view = st.data_editor(view_df, column_config={"選取": st.column_config.CheckboxColumn("選取", default=False, required=True)}, use_container_width=True, key="list_editor_v75")
...
with out_c2:
    ...
    w_num = st.number_input("打字輸入", 1.0, 100.0, float(cur_w), step=0.1, key="w_num_input")
    w_sl = st.slider("滑桿拖動", 1.0, 100.0, float(w_num), step=0.1, key="w_sl_input", label_visibility="collapsed")
...
if st.button("🔥 開始批量生成", type="primary", use_container_width=True):
    if not final_selected_ids:
        ...
        status.text("✅ 生成任務已完成！")
        st.download_button("📥 下載生成的壓縮包 (ZIP)", zip_buf.getvalue(), "output_v7_5.zip", "application/zip", use_container_width=True)
